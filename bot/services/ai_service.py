...
── FIX LOG (patched) ──────────────────────────────────────────────────────────
1. ARA Hunter reason text unconditionally claimed "Price surged >5% above
   previous close" even when the actual gain was within the screener's own
   3.0%-9.0% hard-gate range (e.g. a 3.2% mover would get a false ">5%"
   claim). Now reflects the real pct_chg, conditionally.
2. BSJP reason text unconditionally appended "Net foreign buy streak ≥2 days
   detected" for every single BSJP pass, regardless of whether the
   underlying foreign-buy proxy check in bsjp.py actually fired. Now only
   shown when the proxy condition is genuinely met, and phrased as an
   estimate rather than a detected fact.
3. "Bandar Status" / "Foreign Flow" lines in generate_full_analysis() now
   note these are estimates, since neither is sourced from real IDX
   broker/foreign data (see broker_analyzer.py and screener_engine.py).
"""
...
def generate_screener_reason(stock: dict, screener_type: str) -> str:
    ...
    foreign = stock.get("foreign_flow", "N/A")
    vwap = stock.get("vwap")
    macd = stock.get("macd")
    ...
    if screener_type == "ara_hunter":
        if pct_chg > 5:
            reasons.append(f"📌 Price surged {pct_chg:.1f}% above previous close")
        elif pct_chg > 0:
            reasons.append(f"📌 Price up {pct_chg:.1f}% — within ARA momentum range")
        ...
        # Only claim the foreign-buy proxy signal when it's actually true —
        # previously this line was printed unconditionally for every result.
        foreign_signal = pct_chg > 1.5 and rel_vol >= 1.5
        if foreign_signal:
            reasons.append("📌 Price+volume pattern consistent with foreign buy accumulation (estimate)")
    ...
    if "Accumulation" in broker_sig:
        reasons.append(f"📌 Broker flow (estimate): {broker_sig} — big players loading")
...
def generate_full_analysis(stock: dict, screener_type: str) -> str:
    ...
    return "\n".join([
        ...
        f"*Sector:* {sector}",
        f"*Bandar Status (estimate):* {broker_sig}",
