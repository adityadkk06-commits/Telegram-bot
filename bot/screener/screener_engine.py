...
── FIX LOG (patched) ──────────────────────────────────────────────────────────
1. "foreign_flow" used to be derived from pct_chg (`"Positive" if pct_chg > 1
   else "Neutral"`) — a fabricated label with no actual foreign-flow data
   behind it. Now honestly reported as "N/A" since IDX foreign flow requires
   a paid data feed the bot doesn't have. (BSJP's own foreign-buy PROXY is a
   separate, clearly-labeled estimate inside bsjp.py — left as-is since
   it's transparent about being an approximation.)
2. generate_full_analysis() used to run for EVERY non-fail candidate before
   truncation — e.g. 40+ AI-analysis calls for a run that only ever shows
   8 pass + 5 near. Now it only runs on the final truncated list, cutting
   AI-generation calls dramatically and reducing GitHub Actions timeout risk.
3. Filter errors now logged at WARNING (was DEBUG, invisible at the bot's
   configured INFO log level — real scoring errors were silently swallowed).
"""
        ...
        try:
            ...
            enriched = {
                **stock,
                ...
                "volume_score": vol_sc,
                # Honest: IDX foreign flow requires a paid data feed we don't
                # have. Was previously faked from pct_chg — now reported as
                # N/A rather than a misleading derived label.
                "foreign_flow": "N/A",
            ...
            # WARNING (not debug) — filter errors need to be visible at the
            # bot's default INFO log level, not silently swallowed.
            logger.warning(f"Filter error for {stock.get('ticker')}: {e}")

    passes.sort(key=lambda x: (x["momentum_score"], x["filter_pct"]), reverse=True)
    nears.sort(key=lambda x: x["filter_pct"], reverse=True)

    passes = passes[:max_pass]
    nears = nears[:max_near]

    # AI analysis only for the FINAL truncated list — avoids generating
    # analysis text for dozens of candidates that get discarded anyway.
    for item in passes + nears:
        try:
            item["ai_analysis"] = generate_full_analysis(item, screener_type)
        except Exception as e:
            logger.warning(f"AI analysis error for {item.get('ticker')}: {e}")
            item["ai_analysis"] = ""

    return {"pass": passes, "near": nears}